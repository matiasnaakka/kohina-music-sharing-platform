...
export default function Profile({ session, player }) {
  ...
  const [exportExpiresIn, setExportExpiresIn] = useState(null)

  const GDPR_EXPORT_URL = 'https://mpucvjyqvjxrnjgzpyjb.supabase.co/functions/v1/gdpr-export'

  const getAccessToken = async () => {
    const tokenFromProp = session?.access_token
  ...
  const filenameFromContentDisposition = (value) => {
    ...
    // handles: attachment; filename="export.json"
  ...
  const handleGdprExport = async () => {
    ...
    setExportExpiresIn(null)
    setExportLoading(true)
    try {
      const token = await getAccessToken()
      if (!token) throw new Error('Authentication required')

      const res = await fetch(GDPR_EXPORT_URL, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        // Do NOT send user_id; server must derive user from JWT.
        body: JSON.stringify({}),
      })
      ...
      setExportExpiresIn(payload.expires_in ?? null)
    } catch (err) {
  ...
  // UX: reset export UI when settings closes
  useEffect(() => {
    if (!showSettings) {
      setExportLoading(false)
      setExportError(null)
            ...
            <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
              <div className="w-full max-w-lg mx-4 space-y-4">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
                  <h4 className="text-lg font-semibold mb-2">Download your data</h4>
                  <p className="text-sm text-gray-300 mb-3">
                    Export a copy of your account data from the database.
                  </p>

                  {exportExpiresIn != null && (
                    <p className="text-xs text-gray-400 mb-3">
                      Download links expire in {exportExpiresIn} seconds. Download immediately.
                    </p>
                  )}

                  {exportError && (
                    <div className="bg-red-500 bg-opacity-25 text-red-100 p-2 rounded mb-3 text-sm">
                      {exportError}
                    </div>
                  )}
                  ...
                  >
                    {exportLoading ? 'Preparing export…' : 'Generate download links'}
                  </button>

                  {exportManifest?.files?.length > 0 && (
                    <div className="mt-4">
                      <h5 className="text-sm font-semibold mb-2">Files</h5>
                      <ul className="space-y-2">
                        {exportManifest.files.map((f, i) => (
                          <li key={f?.name || f?.path || i} className="flex items-center justify-between gap-3 bg-gray-800/60 rounded px-3 py-2">
                            <div className="min-w-0">
                              <div className="text-sm truncate">{f?.name || 'export-file'}</div>
                              {f?.size != null && (
                            ...
                            {f?.url ? (
                              <a
                                ...
                                className="shrink-0 text-sm px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600"
                              >
                                Download
                              </a>
                            ) : (
                              <span className="shrink-0 text-xs text-gray-400">No URL</span>
                            )}
                          </li>
                        ))}
                      </ul>
